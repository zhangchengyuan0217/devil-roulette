...
function initPlayers(state, actors, mode) {
  ...
  logState(
    state,
    `游戏开始（${state.mode === "team" ? "组队" : "混战"}）。角色已发放。`,
  );
  startRound(state);
}
...
function remapActorNr(state, oldNr, newNr) {
  ...
  if (state.handDiscardQueue)
    state.handDiscardQueue = state.handDiscardQueue.map(swap);
  if (state.rattlesnakeQueue)
    state.rattlesnakeQueue = state.rattlesnakeQueue.map(swap);
  ...
  if (state.effects) {
    if (Array.isArray(state.effects.linkedPairs)) {
      state.effects.linkedPairs = state.effects.linkedPairs.map((pair) =>
        pair.map(swap),
      );
    }
    if (
      state.effects.doubleItemBonus &&
      state.effects.doubleItemBonus[from] != null
    ) {
  ...
  if (
    ...
    state.rattlesnakeUsedThisRound[from] != null
  ) {
  ...
  if (
    state.awaiting &&
    state.awaiting.targetActorNr != null &&
    Number(state.awaiting.targetActorNr) === from
  ) {
...
function resumeAfterHandDiscard(state) {
  ...
  if (state.pendingAmmoDraw) {
    ...
    state.awaiting = stampAwaitingExpiry(
      Object.assign({ type: "ammo_draw" }, pad),
      "ammo_draw",
    );
...
function discardHandCards(state, actorNr, indexes) {
  ...
  if (uniq.some((i) => i < 0 || i >= p.hand.length))
    return { ok: false, reason: "弃牌无效" };
  const names = [];
  uniq.forEach((i) => {
...
function timeoutDiscardHand(state) {
  ...
  const idxs = [];
  for (let i = p.hand.length - 1; i >= 0 && idxs.length < need; i--)
    idxs.push(i);
  if (idxs.length !== need) {
  ...
  logState(
    state,
    `${p.name} 弃牌超时，系统代为弃置手牌末尾 ${idxs.length} 张。`,
  );
...
function timeoutDoubleBarrel(state) {
  ...
  logState(
    state,
    `${player ? player.name : "#" + actorNr}（双管）超时，本轮不声明。`,
  );
  state.awaiting = null;
  beginTurns(state);
}
...
function startRound(state) {
  ...
  // 装弹：先抽实弹数，再抽空弹数
  if (state.ammoCountDeck.length === 0)
    state.ammoCountDeck = buildAmmoCountDeck();
  ...
  if (state.round === 1) {
    ...
    if (idx > 0) {
      state.turnOrder = state.turnOrder
        .slice(idx)
        .concat(state.turnOrder.slice(0, idx));
    } else if (idx < 0) {
      state.turnOrder = [starter.actorNr].concat(
        state.turnOrder.filter((n) => n !== starter.actorNr),
      );
    }
    state.currentTurn = 0;
    firstPlayer = {
...
function proceedRoundAfterSetup(state) {
  ...
  if (db && state.magazine.length > 0) {
    ...
    logState(
      state,
      `${db.name}（双管）请声明第几发是实弹或空弹（限时 ${state.awaiting.seconds} 秒）。`,
    );
    return;
  }
  beginTurns(state);
...
function onTurnStart(state, player) {
  logState(
    state,
    `轮到 ${player.name} 行动。弹药剩余 ${state.magazine.length}。`,
  );
}
...
function applyDamage(state, target, amount, sourceMsg, ctx) {
  ...
  const damaged = [];
  victims.forEach((v) => {
    if (ctx && ctx.liveBullet && absorbLiveBulletWithVest(state, v, pierceVest))
      return;
    v.hp -= amount;
    damaged.push(v);
    logState(
      state,
      `${sourceMsg || ""}${v.name} 受到 ${amount} 点伤害（剩余 ${Math.max(v.hp, 0)}）。`,
    );
    if (v.hp <= 0) {
      v.hp = 0;
      v.alive = false;
  ...
  if (pairIdx >= 0 && damaged.length > 0) {
    ...
    logState(
      state,
      victims.length > 1 ? "连锁已触发并解除。" : "连锁因受到伤害而解除。",
    );
  }
...
function shoot(state, actorNr, targetActorNr, nightOwlTargetNr) {
  if (state.phase !== "playing" || state.awaiting)
    return { ok: false, reason: "当前不能开枪" };
  ...
  if (
    ...
    if (!picked || !isOpponent(state, shooter, picked)) {
      const foes = alivePlayers(state).filter((p) =>
        isOpponent(state, shooter, p),
      );
...
function continueAfterShotReveal(state) {
  ...
  if (resolved === BULLET.SPECIAL) {
    ...
    if (toSelf && shooter.role === "night_owl" && shooter.hp <= 4) {
      let foe =
        nightOwlTargetNr != null ? getPlayer(state, nightOwlTargetNr) : null;
      ...
      pairIndexes.forEach((idx) => {
        state.effects.linkedPairs[idx].forEach((nr) => {
          ...
          if (
            p &&
            p.alive &&
            !victims.some((v) => Number(v.actorNr) === Number(p.actorNr))
          ) {
            victims.push(p);
          }
        });
      ...
      victims.forEach((v) => {
        ...
        logState(
          state,
          `夜枭实弹：${v.name} -${amount}（剩余 ${Math.max(v.hp, 0)}）`,
        );
        if (v.hp <= 0) {
          v.hp = 0;
          v.alive = false;
      ...
      const primaryHit = damaged.some(
        (v) => Number(v.actorNr) === Number(target.actorNr),
      );
      if (primaryHit && target.alive) {
        target.hp -= 1;
        logState(
          state,
          `追香讨债追加：${target.name} 再受到 1 点伤害（剩余 ${Math.max(target.hp, 0)}）。`,
        );
        if (target.hp <= 0) {
          target.hp = 0;
          target.alive = false;
  ...
  switch (itemId) {
    ...
    case "swap": {
      ...
      if (
        i < 0 ||
        j < 0 ||
        i >= state.magazine.length ||
        j >= state.magazine.length
      ) {
        return "换弹位置无效";
      }
      break;
    ...
    case "steal": {
      const t = getPlayer(state, payload.targetActorNr);
      if (!isOpponent(state, player, t))
        return "顺手牵羊目标无效（需选择有手牌的对手）";
...
function useItem(state, actorNr, itemIndex, payload) {
  if (state.phase !== "playing" || state.awaiting)
    return { ok: false, reason: "当前不能用道具" };
  const player = getPlayer(state, actorNr);
  const cur = currentPlayer(state);
  if (!player || !cur || Number(player.actorNr) !== Number(cur.actorNr)) {
    return { ok: false, reason: "不是你的回合" };
  }
  if (itemIndex < 0 || itemIndex >= player.hand.length)
    return { ok: false, reason: "无效道具" };
...
function beginRattlesnakeWindow(state) {
  ...
  while (state.rattlesnakeQueue && state.rattlesnakeQueue.length) {
    ...
    logState(
      state,
      `${p.name}（响尾蛇）可将【${itemName}】复制到手牌：扣 1 血，限时 ${state.awaiting.seconds} 秒。`,
    );
    return;
  }
  state.awaiting = null;
...
function rattlesnakeCopy(state, actorNr) {
  ...
  const p = getPlayer(state, actorNr);
  if (!p || p.role !== "rattlesnake" || !p.alive)
    return { ok: false, reason: "无法复制" };
  ...
  logState(
    state,
    `${p.name}（响尾蛇）消耗 1 血，将【${itemName}】复制到手牌。`,
  );
...
function rattlesnakePass(state, actorNr) {
  ...
  logState(
    state,
    `${p ? p.name : "#" + actorNr}（响尾蛇）放弃复制【${itemName}】。`,
  );
  state.awaiting = null;
  beginRattlesnakeWindow(state);
  return { ok: true };
...
function rattlesnakeTimeout(state, token) {
  ...
  if (token && state.awaiting.token !== token)
    return { ok: false, reason: "窗口已过期" };
  const p = getPlayer(state, state.awaiting.actorNr);
  const itemName = state.awaiting.itemName || "道具";
  logState(
    state,
    `${p ? p.name : "?"}（响尾蛇）超时，放弃复制【${itemName}】。`,
  );
  state.awaiting = null;
  beginRattlesnakeWindow(state);
  return { ok: true };
...
function continueAfterItemFx(state) {
  ...
  const player = getPlayer(state, actorNr);
  state.awaiting = null;
  if (offerSnake && player)
    offerRattlesnake(state, player, itemId, snakePayload || {});
  if (!state.awaiting) tryStartHandDiscard(state);
  return { ok: true };
}
  ...
  switch (itemId) {
    ...
    case "inspect": {
      ...
      if (idx >= 0 && idx < state.magazine.length) {
        ...
        logState(
          state,
          `${player.name} 检视了弹药堆从上数第 ${idx + 1} 张（仅自己可见）。`,
        );
        startItemFx(state, player, itemId, payload, {
          private: true,
          peekIndex: idx,
    ...
    case "swap": {
      ...
      if (
        a >= 0 &&
        b >= 0 &&
        a < state.magazine.length &&
        b < state.magazine.length
      ) {
        [state.magazine[a], state.magazine[b]] = [
          state.magazine[b],
          state.magazine[a],
        ];
        ...
        logState(
          state,
          `${player.name} 交换了第 ${i + 1} 发与第 ${j + 1} 发。`,
        );
        startItemFx(state, player, itemId, payload, {
          swapFrom: i + 1,
          swapTo: j + 1,
    ...
    case "steal": {
      ...
      logState(
        state,
        `${player.name} 从 ${t.name} 处随机抽走了【${stolenName}】。`,
      );
      startItemFx(state, player, itemId, payload, {
        targetName: t.name,
        targetActorNr: t.actorNr,
    ...
    case "dismantle": {
      ...
      if (!isOpponent(state, player, t) || !t.hand.length) break;
      const idx = Math.floor(Math.random() * t.hand.length);
      const dumped = t.hand.splice(idx, 1)[0];
      state.itemDiscard.push(dumped);
      const dumpedName = (ITEMS[dumped] && ITEMS[dumped].name) || dumped;
      logState(
        state,
        `${player.name} 使 ${t.name} 随机弃置了【${dumpedName}】。`,
      );
      startItemFx(state, player, itemId, payload, {
        targetName: t.name,
        targetActorNr: t.actorNr,
        discardedItemId: dumped,
        discardedName: dumpedName,
      });
      break;
    }
    case "double_arrow": {
      if (payload.unlock) {
        ...
        const targets = (payload.targets || [])
          .map((nr) => getPlayer(state, nr))
          .filter((p) => p && p.alive);
        const ids = [...new Set(targets.map((p) => Number(p.actorNr)))].slice(
          0,
          2,
        );
        ...
        logState(
          state,
          `连锁目标：${targets.map((p) => p.name).join("、") || "无"}。`,
        );
        startItemFx(state, player, itemId, payload, {
          unlock: false,
...
function declareDoubleBarrel(state, actorNr, index1Based, kind) {
  ...
  if (idx < 0 || idx >= state.magazine.length)
    return { ok: false, reason: "发数无效" };
  if (kind !== BULLET.LIVE && kind !== BULLET.BLANK)
    return { ok: false, reason: "只能声明实/空" };
  ...
  const allies = alivePlayers(state).filter(
    (p) => Number(p.team) === Number(player.team),
  );
  const foes = alivePlayers(state).filter(
    (p) => Number(p.team) !== Number(player.team),
  );
  const gainers = success ? allies : foes;
  gainers.forEach((p) => {
    state.effects.doubleItemBonus[p.actorNr] =
      (state.effects.doubleItemBonus[p.actorNr] || 0) + 2;
  });
  logState(
    state,
    `${success ? "己方" : "对手"} 下轮将额外获得双倍道具（+2）。`,
  );
...
function skipDoubleBarrel(state, actorNr) {
  ...
  logState(
    state,
    `${player ? player.name : "#" + actorNr}（双管）选择本轮不声明。`,
  );
  state.awaiting = null;
  beginTurns(state);
  return { ok: true };
...
function useIronRose(state, actorNr, handIndexes) {
  ...
  const cur = currentPlayer(state);
  if (!cur || Number(cur.actorNr) !== Number(actorNr))
    return { ok: false, reason: "不是你的回合" };
  if (!Array.isArray(handIndexes) || handIndexes.length !== 2)
    return { ok: false, reason: "需弃两张" };
  const idxs = handIndexes.slice().sort((a, b) => b - a);
  if (idxs[0] === idxs[1] || idxs.some((i) => i < 0 || i >= p.hand.length))
    return { ok: false, reason: "手牌无效" };
  ...
  alivePlayers(state)
    .filter(
      (x) =>
        Number(x.team) === Number(p.team) &&
        Number(x.actorNr) !== Number(p.actorNr),
    )
    .forEach((ally) => {
      heal(state, ally, 1);
...
function publicView(state, viewerActorNr) {
  ...
  return {
    ...
    lastItemForSnake: state.lastItemForSnake
      ? {
          itemId: state.lastItemForSnake.itemId,
          from: state.lastItemForSnake.from,
        }
      : null,
  };
  ...
  switch (action.type) {
    case "shoot":
      return shoot(
        state,
        actorNr,
        action.targetActorNr,
        action.nightOwlTargetNr,
      );
    case "use_item":
